...
class Puzzle:
    ...
    def _enque(self, new_state):    #enqueuing in open list
        ...
        x = self._heu_el(new_state)   #storing last value in a variable

        if len(self.queue) == 0:      #if queue is empty, add new state at first
            self.queue.append(new_state)

        elif x < self._heu_el(self.queue[0]):   #if heuristic value of first element is less than x state, add it at first
    ...
    def _heu_el(self, state):   #heuristic function (Eucleadian)
        val = 0
                ...
                for j in range(3):
                    for k in range(3):
                        if q == self.goal[j][k] and not (x == j and i == k):
                            val += pow(abs(x-j),2)+pow(abs(i-k),2)  #Eucleadian Formula with power
                            break
        return pow(val,1/2)   #return Eucleadian distance
